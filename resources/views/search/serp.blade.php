								...
								<div class="breadcrumb-list text-center-xs">
									...
									@if ($isLocSearch or Request::has('l') or Request::has('r'))
										...
										<a rel="nofollow" class="jobs-s-tag" href="{!! $searchUrl !!}">
											{!! t(':distance km around :city', ['distance' => \App\Larapen\Helpers\Search::$distance, 'city' => $city->name]) !!}
										</a>
									@endif
									...
									@if (Request::has('type'))
										@if (is_array(Request::get('type')))
											@foreach(Request::get('type') as $key => $value)
												<?php
												$jobType = \App\Larapen\Models\AdType::find($value);
												?>
												@if (!empty($jobType))
										...
										@else
											<?php
											$jobType = \App\Larapen\Models\AdType::find(Request::get('type'));
											?>
											@if (!empty($jobType))
