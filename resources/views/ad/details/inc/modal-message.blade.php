		...
		<div class="modal-content">
			...
			<form role="form" method="POST" action="{{ lurl($ad->id . '/contact') }}" enctype="multipart/form-data">
				...
				<div class="modal-body">
					...
					<!-- Resume -->
					<div class="form-group required <?php echo ($errors->has('resume')) ? 'has-error' : ''; ?>">
						<label for="resume" class="control-label">{{ t('Resume') }} </label>
						<input id="resume" name="resume" type="file" class="file">
						<p class="help-block">{{ t('Resume format') }}</p>
						@if (!empty($resume) and file_exists(public_path() . '/uploads/resumes/' . $resume->filename))
							<div>
								<a class="btn btn-default" href="{{ url('uploads/resumes/' . $resume->filename) }}" target="_blank">
...
@section('javascript')
	...
	<script language="javascript">
		...
		{
			'showPreview': false,
			'allowedFileExtensions': ['pdf', 'doc', 'docx', 'rtf', 'odt', 'png', 'jpg', 'jpeg'],
