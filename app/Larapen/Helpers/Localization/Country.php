...
class Country
{
    ...
    // Maxmind Support URL
    private static $maxmind_support_url = 'http://www.bedigit.com/guide/jobclass/';
    ...
    public function setCountryParameters()
    {
        ...
        if (in_array(trans('routes.' . config('larapen.localization.countries_list_uri')), [getSegment(1)])) {
            return false;
        }
            ...
            if (config('settings.activation_geolocation')) {
                if (Auth::check()) {
                    ...
                    if ($user->is_admin == 1) {
                        ...
                        $msg .= "Please check the <a href='" . self::$maxmind_support_url . "' target='_blank'>Maxmind database installation for JobClass</a> support.";
