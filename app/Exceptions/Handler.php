    ...
    public function render($request, Exception $e)
    {
        ...
        if ($e instanceof \PDOException) {
            ...
            if (in_array($e->getCode(), $tableErrorCodes['mysql']) or in_array($e->getCode(), $tableErrorCodes['standardized'])) {
                $msg = '';
                $msg .= '<html><head><title>Installation - JobClass</title></head><body>';
                $msg .= '<pre>';
                ...
                $msg .= '<br>-- Please replace <<prefix>> by your database tables prefix before importing theses SQL files';
                $msg .= '<ul>';
                $msg .= '<li><code>/storage/database/sql/schema.sql</code> (required)</li>';
                $msg .= '<li><code>/storage/database/sql/data.sql</code> (required)</li>';
                $msg .= '<li><code>/storage/database/sql/geonames/[country_code].sql</code> (required)</li>';
                $msg .= '</ul>';
